...
def get_transform_mat(image_landmarks, output_size, face_type, scale=1.0):
    ...
    if remove_align:
        bbox = transform_points(
            [(0, 0), (0, output_size - 1), (output_size - 1, output_size - 1), (output_size - 1, 0)], mat, True)
...
def draw_landmarks(image, image_landmarks, color=(0, 255, 0), transparent_mask=False, ie_polys=None, extend_forehead=False):
    if len(image_landmarks) != 68:
        raise Exception('get_image_eye_mask works only with 68 landmarks')
...
def draw_rect_landmarks(image, rect, image_landmarks, face_size, face_type, transparent_mask=False, ie_polys=None,
                        extend_forehead=False, landmarks_color=(0, 255, 0)):
