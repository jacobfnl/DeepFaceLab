...
from imagelib.color_transfer import ColorTransferMode, lab_decimal_to_bgr
...
class SAEHDModel(ModelBase):
    ...
    def onGetPreview(self, sample):
        ...
        S, D, SS, DD, SD = ([{
            ...
            'H': lab_decimal_to_bgr(img, l_crop=(0.5, 0.5)),
            'LA': lab_decimal_to_bgr(img, b_crop=(0.5, 0.5)),
            'LB': lab_decimal_to_bgr(img, a_crop=(0.5, 0.5)),
        ...
        if self.options['learn_mask']:
            ...
            for i in range(len(test_S)):
                for ch in ['BGR']:
                    ...
                    ar_over = S[i][ch], SS[i][ch]*SSM[i] + S[i][ch]*(1-SSM[i]), D[i][ch], DD[i][ch]*DDM[i] + D[i][ch]*(1-DDM[i]), SD[i][ch]*(1-(1-DDM[i])*(1-SDM[i])) + D[i][ch]*(1-DDM[i])*(1-SDM[i])
