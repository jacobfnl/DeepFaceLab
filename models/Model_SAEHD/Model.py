...
class SAEHDModel(ModelBase):
    ...
    def onInitializeOptions(self, is_first_run, ask_override):
        ...
        if is_first_run or ask_override:
            ...
            default_loss_scale = self.options.get('loss_scale', 1)
            self.options['loss_scale'] = io.input_number("Loss scale (skip:%.2f) : " % default_loss_scale, default_loss_scale)
            ...
            self.options['loss_scale'] = self.options.get('loss_scale', 1)
    ...
    def onInitialize(self):
        ...
        if self.is_training_mode:
            self.src_dst_opt      = LossScaleOptimizer(RMSprop(lr=5e-5, clipnorm=1.0 if self.options['clipgrad'] else 0.0, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
            self.src_dst_mask_opt = LossScaleOptimizer(RMSprop(lr=5e-5, clipnorm=1.0 if self.options['clipgrad'] else 0.0, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
            self.D_opt            = LossScaleOptimizer(RMSprop(lr=5e-5, clipnorm=1.0 if self.options['clipgrad'] else 0.0, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
