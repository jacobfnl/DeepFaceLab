...
class SAEHDModel(ModelBase):
    ...
    def onInitialize(self):
        ...
        if self.is_training_mode:
            self.src_dst_opt      = LossScaleOptimizer(RMSprop(lr=5e-5, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
            self.src_dst_mask_opt = LossScaleOptimizer(RMSprop(lr=5e-5, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
            self.D_opt            = LossScaleOptimizer(RMSprop(lr=5e-5, tf_cpu_mode=self.options['optimizer_mode']-1), 'dynamic')
