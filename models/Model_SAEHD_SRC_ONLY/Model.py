...
class SAEHDModel(ModelBase):
    ...
    def onInitialize(self):
        ...
        class SAEDFModel(CommonModel):
            def __init__(self, resolution, ae_dims, e_ch_dims, d_ch_dims, learn_mask):
                ...
                if learn_mask:
                    self.src_mask_trainable_weights = self.encoder.trainable_weights + self.decoder_srcm.trainable_weights

                self.warped_src = Input(bgr_shape)
                self.target_src = Input(bgr_shape)
                self.target_srcm = Input(mask_shape)
        ...
        class SAELIAEModel(CommonModel):
            def __init__(self, resolution, ae_dims, e_ch_dims, d_ch_dims, learn_mask):
                ...
                sh = np.array(K.int_shape( self.inter_AB.outputs[0] )[1:])*(1,1,2)
                ...
                self.src_trainable_weights = self.encoder.trainable_weights + self.inter_AB.trainable_weights + self.decoder.trainable_weights

                if learn_mask:
                    self.src_mask_trainable_weights = self.encoder.trainable_weights + self.inter_AB.trainable_weights + self.decoderm.trainable_weights
        ...
        if self.is_training_mode:
            self.src_opt      = RMSprop(lr=5e-5, clipnorm=1.0 if self.options['clipgrad'] else 0.0, tf_cpu_mode=self.options['optimizer_mode']-1)
            self.src_mask_opt = RMSprop(lr=5e-5, clipnorm=1.0 if self.options['clipgrad'] else 0.0, tf_cpu_mode=self.options['optimizer_mode']-1)
            ...
            if background_power != 0:
                if self.options['ms_ssim_loss']:
                    ...
                    src_loss += K.mean( (10*background_power)*K.square( pred_src_src_anti_masked - target_src_anti_masked ))


            G_loss = src_loss

            self.src_train = K.function ([self.model.warped_src, self.model.target_src, self.model.target_srcm],
                                             [src_loss],
                                             self.src_opt.get_updates( G_loss, self.model.src_trainable_weights)
                                             )
            ...
            if self.options['learn_mask']:
                ...
                self.src_mask_train = K.function ([self.model.warped_src, self.model.target_srcm],[src_mask_loss], self.src_mask_opt.get_updates(src_mask_loss, self.model.src_mask_trainable_weights ) )

            if self.options['learn_mask']:
                self.AE_view = K.function ([self.model.warped_src], [self.model.pred_src_src, self.model.pred_src_srcm])
            else:
                self.AE_view = K.function ([self.model.warped_src], [self.model.pred_src_src])

        else:
            if self.options['learn_mask']:
                self.AE_convert = K.function ([],[])
            else:
                self.AE_convert = K.function ([],[])
        ...
        if self.is_training_mode:
            ...
            self.set_training_data_generators ([
                    SampleGeneratorFace(training_data_src_path, debug=self.is_debug(), batch_size=self.batch_size,
    ...
    def get_model_filename_list(self):
        return self.model.get_model_filename_list ( exclude_for_pretrain=(self.pretrain and self.iter != 0) )
    #override
    def onSave(self):
    ...
    def onTrainOneIter(self, generators_samples, generators_list):
        ...
        feed = [warped_src, target_src, target_srcm]

        src_loss, = self.src_train (feed)
    ...
    def onGetPreview(self, sample):
        ...
        if self.options['learn_mask']:
            ...
            S, SS = [ np.clip(x, 0.0, 1.0) for x in ([test_S] + self.AE_view ([test_S]) ) ]
        ...
        result += [ ('SAEHD_SRC_ONLY', np.concatenate (st, axis=0 )), ]
        ...
        if self.options['learn_mask']:
            ...
            result += [('SAEHD_SRC_ONLY overlay', np.concatenate(st_p, axis=0)), ]

        return result

    def predictor_func (self, face=None, dummy_predict=True):
