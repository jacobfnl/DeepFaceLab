...
class MaskEditor:
    ...
    def make_screen(self):
        ...
        if self.preview_images is None:
            ...
            prev_images = [(t, cv2.resize(imagelib.normalize_channels(img, 3), max_wh_bordered)) for t, img in
                           prev_images]
            next_images = [(t, cv2.resize(imagelib.normalize_channels(img, 3), max_wh_bordered)) for t, img in
                           next_images]
...
def mask_editor_main(input_dir, confirmed_dir=None, skipped_dir=None):
    ...
    while not is_exit:
        ...
        def get_status_lines_func():
            return ['Progress: %d / %d . Current file: %s' % (
            len(done_paths), image_paths_total, str(filepath.name) if filepath is not None else "end"),
